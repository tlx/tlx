...
auto foldr_impl(Reduce &&r, Init &&init, Arg &&arg)
{
    return std::forward<Reduce>(r)(std::forward<Arg>(arg), std::forward<Init>(init));
}

//! helper for foldr: general recursive case
template <typename Reduce, typename Init, typename Arg, typename... MoreArgs>
auto foldr_impl(Reduce&& r, Init&& init, Arg&& arg, MoreArgs&&... rest)
{
...
auto foldr(Reduce&& r, Init&& init, Args&&... args)
{
    return detail::foldr_impl(std::forward<Reduce>(r), std::forward<Init>(init),
